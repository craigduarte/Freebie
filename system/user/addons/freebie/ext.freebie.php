...
class Freebie_ext {
	...
	*/
	...
	{
		// EE global instance called by ee(); and extension settings
	...
	*/
	function should_execute(){
		...
		strtolower(substr(ee()->uri->uri_string, 0,3)) != 'cp/' &&
	...
	*/
	function clear_cache(){
		...
		if ( ! isset( $db_settings['cache_cleared'] ) ) {
			...
			$sql = ee()->db->update_string('exp_extensions', $data, "class = 'Freebie_ext'");
			ee()->db->query($sql);
